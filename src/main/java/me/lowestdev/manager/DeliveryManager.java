...
public class DeliveryManager {
	...
	public synchronized boolean hasDelivery(String playerName) {
		try (PreparedStatement stmt = connection
			...
			plugin.getLogger().info(
					"Consulta de entregas para " + playerName + ": " + (has ? "possui entregas" : "nenhuma entrega"));
			return has;
		} catch (SQLException e) {
	...
	public synchronized Set<String> getPendingPlayers() {
		...
		try (PreparedStatement stmt = connection.prepareStatement("SELECT DISTINCT player_name FROM deliveries")) {
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
		...
		try {
			...
			try (PreparedStatement stmt = connection
				...
				while (rs.next()) {
					...
					deserializeItems(rs.getString("items"));
