...
public class DeliveryManager {
	...
	public synchronized void addDelivery(String targetPlayerName, List<ItemStack> items, String senderName) {
		executeTransaction(connection -> {
			try (PreparedStatement stmt = connection
					.prepareStatement("INSERT INTO deliveries (player_name, sender_name, items) VALUES (?, ?, ?)")) {
				...
				org.bukkit.entity.Player senderPlayer = Bukkit.getPlayerExact(senderName);
	...
	public synchronized boolean hasDelivery(String playerName) {
		try (Connection connection = dataSource.getConnection();
				PreparedStatement stmt = connection
						.prepareStatement("SELECT COUNT(*) FROM deliveries WHERE player_name = ?")) {
	...
	public synchronized List<DeliverySlot> getDeliveries(String playerName) {
		...
		try (Connection connection = dataSource.getConnection();
				PreparedStatement stmt = connection
						.prepareStatement("SELECT id, sender_name, items FROM deliveries WHERE player_name = ?")) {
			stmt.setString(1, playerName.toLowerCase(Locale.ROOT));
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
				int id = rs.getInt("id");
	...
	/** Returns the items for a specific delivery id, or null if not found. */
	public synchronized List<ItemStack> getDeliveryItems(int deliveryId) {
		try (Connection connection = dataSource.getConnection();
				PreparedStatement stmt = connection.prepareStatement("SELECT items FROM deliveries WHERE id = ?")) {
			...
			plugin.getLogger().log(Level.SEVERE, "Erro ao buscar itens de entrega", e);
			return null;
		}
	}

	/**
	 * Update a delivery's items (replace with updatedItems). If updatedItems is
	 * empty the delivery is deleted.
	 */
	public synchronized void updateDeliveryItems(int deliveryId, List<ItemStack> updatedItems) {
		executeTransaction(connection -> {
			if (updatedItems == null || updatedItems.isEmpty()) {
				...
				try (PreparedStatement stmt = connection
						.prepareStatement("UPDATE deliveries SET items = ? WHERE id = ?")) {
	...
	public synchronized int cancelDeliveries(String targetPlayerName, Plugin pluginInstance) {
		...
		executeTransaction(connection -> {
			try (PreparedStatement stmt = connection
					.prepareStatement("SELECT id FROM deliveries WHERE player_name = ?")) {
				stmt.setString(1, targetPlayerName.toLowerCase(Locale.ROOT));
				ResultSet rs = stmt.executeQuery();
				while (rs.next()) {
					int id = rs.getInt("id");
	...
	public synchronized int getItemCount(String playerName) {
		...
		try (Connection connection = dataSource.getConnection();
				PreparedStatement stmt = connection
						.prepareStatement("SELECT items FROM deliveries WHERE player_name = ?")) {
			stmt.setString(1, playerName.toLowerCase(Locale.ROOT));
			ResultSet rs = stmt.executeQuery();
			while (rs.next()) {
				List<ItemStack> items = deserializeItems(rs.getString("items"));
	...
	public synchronized void removeDeliveryById(int deliveryId) {
		executeTransaction(connection -> {
			try (PreparedStatement stmt = connection.prepareStatement("DELETE FROM deliveries WHERE id = ?")) {
				stmt.setInt(1, deliveryId);
				stmt.executeUpdate();
			}
		});
